...
│                 主线程（Main）               │
│----------------------------------------------│
│ 捕获视频帧 → 生成任务(task_id, frame_data)  │
│ 加锁(mutex)                                  │
│   ┌──────────────────────────────────┐       │
│   │ 任务队列 push(task)              │  ← 临界区（受互斥锁保护）
│   └──────────────────────────────────┘       │
│ 解锁(mutex)                                  │
│  ↓ 通知工作线程(condition_variable.notify)   │
└──────────────────────────────────────────────┘
                         │
                         ▼
┌──────────────────────────────────────────────┐
...
│   等待条件变量唤醒                            │
│   加锁(mutex)                                │
│     从任务队列 pop(task)                     │ ← 临界区：防止多个线程取同一帧
│   解锁(mutex)                                │
│   推理处理(task.frame_data)                  │
│   加锁(result_mutex)                         │
│     写入共享结果(result_map[task.id])        │ ← 临界区：防止多个线程写入同一结果表
│   解锁(result_mutex)                         │
│   记录日志 + FPS统计                         │
│----------------------------------------------│
│ 线程2、线程3、线程4 …… 同上                  │
